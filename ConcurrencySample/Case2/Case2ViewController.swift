...
class Case2ViewController: UIViewController {
    ...
    private func fetchUserBefore(for id: String, completion: @escaping (User) -> Void) {
    ...
    private func fetchUserAfter(for id: String) async -> User {
...
extension Case2ViewController {
    private func downloadDataBefore(from url: URL, completion: @escaping (Data) -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            completion(self.parseJson(from: "{\"id\":1, \"name\":\"Suzuki\"}")!)
        }
    }
    
    private func downloadDataAfter(from url: URL) async -> Data {
        /// 多分1秒遅延
